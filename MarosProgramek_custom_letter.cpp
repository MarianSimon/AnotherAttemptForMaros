...
{
    //List of all vowels
    static const string input_character  = "aeiouyýáíéúóůěBCČDĎFGHJKLĽĹMNŇPQRŘŔSŠTŤVWXZŽ";
    static const string output_character = "AEIOUYÝÁÍÉÓÚŮĚbcčdďfghjklľĺmnňpqrřŕsštťvwxzž";
    //If no change is done, the convert is dummy
    char modified_c=c;
    string::size_type position=input_character.find(modified_c);
    //Condition for vowel: find(wovel_list.begin(), wovel_list.end(), c) != wovel_list.end()
    if(position != std::string::npos) modified_c=output_character[position];    
    //Return modified c
    return(modified_c);
}
